...
class dynamodbAPI:
    ...
    def init(self, msg_chat_id, role, msg_text):
        Item = {
            "_id": {"N": str(msg_chat_id)},
            "chat_logs": {
                "L": [
                    {"M": {"role": {"S": str(role)}, "content": {"S": str(msg_text)}}}
                ]
            },
        }
        return Item
    ...
    def template(self, msg_chat_id, the_list):
        Item = {"_id": {"N": str(msg_chat_id)}, "chat_logs": {"L": the_list}}
        return Item
    ...
    def put_item(self, Item):
        response = self.client.put_item(TableName=self.tableName, Item=Item)
        return response
    ...
    def convert_regular_dictionary_to_dynamodb(self, chat_history):
        ...
        for i in range(len(chat_history)):
            ...
            formated.append(
                {
                    "M": {
                        "content": {"S": format_me["content"]},
                        "role": {"S": format_me["role"]},
                    }
                }
            )
        return formated
